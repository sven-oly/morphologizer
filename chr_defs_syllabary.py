...
class morph_chr_syllabary(morph_chr_base):
    def __init__(self):
        ...
        self.Grammar["PersonPrefixes"] = fin_person
        ...
        self.Lexicon = FST.rlg(Grammar, "S")
        self.Lexicon = Lexicon.epsilon_remove().determinize().minimize()
        ...
        #metathesis1 = FST.re("$^rewrite((yi\^h):(y\^))", fsts)

        self.metathesis1 = FST.re("$^rewrite((i\^h):(\^) / (y|w|n) _ )", self.fsts)


        #yi^HIy = HYIy^
        metathesself.is2 = FST.re("$^rewrite(Ꭿ:'' / (y|w|n)i\^ _ )", self.fsts)
        ...
        final = FST.re("$Lexicon @ $rule1 @ $rule2 @ $rule2_1 @ $rule3 @ $metathesis1 @ $metathesis2 @ $aspiration1 @ $aspiration2 @ $trn @ $ngt1 @ $ngt2 @ $ngt3 @ $ngt4 @ $ngt5 @ $rule4 @ $rule5 @ $rule6 @ $cleanup1 @ $rule8 @ $rule9 @ $cleanup2 @ $kw @ $k", fsts)


class morph_defs_latin(morph_chr_base):
    def __init__(self):
